...
// mockRunner implements runner.CommandRunner for handler tests.
...
// setupTestHandlers creates Handlers with a test project directory and mock services.
func setupTestHandlers(t *testing.T, runner *mockRunner) (*Handlers, *mux.Router) {
	t.Helper()
	...
	hub := NewHub()
	h := NewHandlers(dir, hub, gitSvc, dockerSvc)
	h.RefreshProjects()
	...
	api.HandleFunc("/projects/{id}/commands", h.ListCommands).Methods("GET")
	api.HandleFunc("/projects/{id}/git/status", h.GitStatus).Methods("GET")
	api.HandleFunc("/projects/{id}/git/commit", h.GitCommit).Methods("POST")
	api.HandleFunc("/projects/{id}/git/stage", h.GitStage).Methods("POST")
	api.HandleFunc("/projects/{id}/docker/containers", h.DockerContainers).Methods("GET")
...
func TestListProjects(t *testing.T) {
	...
	_, router := setupTestHandlers(t, runner)
